use crate::update::components::CameraProjection;
use alexandria::math::Vector3f;

impl CameraProjection {
    /// Get the eight corners that make up this projection
    pub(in crate::update::components::camera) fn shadow_corners(
        &self,
        aspect: f32,
    ) -> [[Vector3f; 4]; 2] {
        match self {
            &CameraProjection::Perspective { fov_y, near, far } => {
                perspective_corners(aspect, fov_y, near, far)
            }
            &CameraProjection::InfinitePerspective {
                fov_y,
                near,
                shadow_distance,
            } => perspective_corners(aspect, fov_y, near, shadow_distance),
            &CameraProjection::Orthographic { size, near, far } => {
                orthographic_corners(aspect, size, near, far)
            }
        }
    }
}

fn perspective_corners(aspect: f32, fov_y: f32, near: f32, far: f32) -> [[Vector3f; 4]; 2] {
    let tan_fov = (fov_y / 2.0).tan();

    [
        perspective_plane_corners(aspect, tan_fov, near),
        perspective_plane_corners(aspect, tan_fov, far),
    ]
}

fn perspective_plane_corners(aspect: f32, tan_fov: f32, depth: f32) -> [Vector3f; 4] {
    let corner = perspective_corner(aspect, tan_fov, depth);
    [
        corner,
        Vector3f::new(-corner.x, corner.y, corner.z),
        Vector3f::new(corner.x, -corner.y, corner.z),
        Vector3f::new(-corner.x, -corner.y, corner.z),
    ]
}

fn perspective_corner(aspect: f32, tan_fov: f32, depth: f32) -> Vector3f {
    Vector3f::new(aspect * tan_fov * depth, tan_fov * depth, depth)
}

fn orthographic_corners(aspect: f32, size: f32, near: f32, far: f32) -> [[Vector3f; 4]; 2] {
    let half_width = size * aspect;

    [
        orthographic_plane_corners(half_width, size, near),
        orthographic_plane_corners(half_width, size, far),
    ]
}

fn orthographic_plane_corners(half_width: f32, half_height: f32, depth: f32) -> [Vector3f; 4] {
    [
        Vector3f::new(half_width, half_height, depth),
        Vector3f::new(-half_width, half_height, depth),
        Vector3f::new(half_width, -half_height, depth),
        Vector3f::new(-half_width, -half_height, depth),
    ]
}
