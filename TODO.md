# ToDo
 1. Add pipeline state wrapper
 2. Add material properties
 3. Improve colors 
    - u8 vs f32 distinction
 4. Add general file I/O thread
 5. Add textures and texture loading
 6. Add shadow maps
 7. Add model loading
 8. Add fixed update
 9. Add bounds to `Mesh`
 10. Add rigidbody
 11. Add colliders
 12. Add physically based rendering
 13. Finish `KeyCode` enum
 14. Add mouse support
 15. Add xbox controller support
 16. Add UI
 17. Add transparency
 18. Add audio

## Additional ToDos
 1. Add post-processing support
    1. Add post-processing "mesh"
    2. Add post-processing vertex shader
    3. Add post-processing sampler
    4. Add color correction
    5. Add `set_render_scale` function
    6. Add FXAA
    7. Add `set_anti_aliasing` function
    9. Add access to the provided post-processing arena
 2. Add SSAO
 3. Add deferred rendering
 4. Add camera frustum culling 
 5. Add light frustum culling
 6. Add light-object culling
 7. Add collider culling
 8. Add toggleable wireframe rendering
 9. Add render scale
 10. Add input binding helper
 11. Add joystick support
 12. Add steering wheel support
 13. Add other controller support
 14. Add occlussion culling
 15. Add multithreaded untracked resource creation + uploading
 16. Reverse message and main thread
 17. Add job system
 18. Jobify rendering
 19. Generalize quaternion look at function
 20. Expand math options
 21. Expand mesh options
 22. Add single creation of primitive meshes
 23. Add sphere primitive
 24. Add capsule primitive
 25. Add pyramid primitive
 26. Allow cameras to render to textures
 27. Add SMAA
 28. Add TAA
 29. Upgrade to Vulkan
 30. Add Linux support
 31. Add device, render context, and command list wrappers